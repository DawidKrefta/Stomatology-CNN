...
model = Model(inputs=mobile.input, outputs=predictions)  # model bez 6 ostatnich warstw
model.summary()
for layer in model.layers[:-5]:
    layer.trainable = False
model.compile(optimizer=Adam(learning_rate=0.0001), loss='categorical_crossentropy', metrics=['accuracy'])
history = model.fit_generator(generator=train_batches, steps_per_epoch=110,
                              validation_data=valid_batches, validation_steps=60, epochs=40, verbose=2)
